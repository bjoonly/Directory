...
public:
	...
	bool SearchValue(string value)const {
		bool res = false;
		for (Node* current = head; current != nullptr; current = current->next)
	...
	template<class T>friend ifstream& operator>>(ifstream& in, List<T>& list);
	template<class T>friend ofstream& operator<<(ofstream& out, const List<T>& list);
...
template<typename T>
ofstream& operator<<(ofstream& out, const List<T>& list) {
	out << list.GetCount()<<endl;
	for (typename List<T>::Node* current = list.head; current != nullptr; current = current->next) {
